...
class Form_builder {
    ...
    private $config = array(/* Config array - can be overrided by passing in array in ini() */
        ...
        'default_button_classes' => 'btn sendbtn btn-primary',
...
//        }
        ...
        if ($this->func == 'form_combine') {
            ...
            switch ($this->func) {
                ...
                case 'form_date':
                   // $this->input_addons['exists'] = true;
                    ...
                    if (empty($this->elm_options['value'])) {
                      ...
                      if( $this->elm_options['value'] == '0000-00-00' ){
                    ...
                    $this->elm_options['ng-model'] = 'row.'.$this->elm_options['id'];
                    $input_html_string = form_date($this->elm_options);
                    break;
                ...
                case 'form_email':
                    ...
                    break;
                case 'form_passwd':
                    $this->elm_options['type'] = 'passwd';
                    $this->elm_options['ng-model'] = 'row.'.$this->elm_options['id'];
                    $input_html_string = form_input($this->elm_options);
                    break;
                case 'form_number':
                    $this->elm_options['type'] = 'number';
                    $this->elm_options['step'] = 'any';
                    $this->elm_options['ng-model'] = 'row.'.$this->elm_options['id'];
                    $input_html_string = form_input($this->elm_options);
                    break;
                case 'form_imagen':
                    ...
                    $options = $this->elm_options;
                    $options['name_span'] = $options['name'].'_preview';
                    $options['name'] = $options['name'].'_file';
                    $options['id'] = $options['id'].'_file';
                    //$options['class'] = 'dropzone';
                    //$input_html_string .= form_upload($options);
                    $input_html_string .= '<div id="image-holder"></div>';

                    if(empty($this->file) ) $input_html_string = '<script>var extension=["jpg","jpeg","png","gif"]; var file = [];</script>';

                    $input_html_string .= form_upload($options).form_hidden($this->elm_options['id'], $this->elm_options['value']).'
                    <script>file['.$this->file.'] = "'.$this->elm_options['name'].'"; </script><div id="queue_'.$this->elm_options['name'].'"></div>';
                    $this->file++;
                    break;
                ...
                case 'form_file':
                    ...
                    if(empty($this->file) ) $input_html_string = '<script>var extension=["xls","pdf","docx","doc","csv"]; var file = [];</script>';
                ...
                case 'form_hidden':
                     $this->elm_options['ng-model'] = 'row.'.$this->elm_options['id'];
                    return form_hidden($this->elm_options);
                case 'form_submit':
        ...
        if ($this->input_addons['exists']) {
            if (!empty($this->input_addons['post_html'])) {
                ...
                foreach ($this->input_addons['post'] as $post_addon) {
                  ...
                  $ret_string .= '<span class="input-group-addon" style="padding:0;background-color:transparent;">' . $post_addon . '</span>';
                }
            }
            $ret_string .= '</div>';
        }
    ...
    private function _pre_elm() {
        $this->cont++;
        return '<div id="divform'.$this->cont.'" class="' . $this->config['default_input_container_class'] . '">';
    }
